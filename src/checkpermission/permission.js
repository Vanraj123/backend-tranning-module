...
const checkcanview = async (req, res, next) => {
  try {
    ...
    if (userPermission && userPermission.can_view === true) {
      return next();
    } else {
      return res.status(403).json({ status: "error", message: "User does not have view permission" });
    }
  } catch (err) {
    console.log(err);
...
const checkcancreate = async (req, res, next) => {
  try {
    ...
    if (userPermission && userPermission.can_create === true) {
      ...
      return res.status(403).json({ status: "error", message: "User does not have create permission" });
    }
  } catch (err) {
    console.log(err);
    return res.status(500).json({ status: "error", message: "Failed to fetch permission" });
...
const checkcanupdate = async (req, res, next) => {
  try {
    ...
    if (userPermission && userPermission.can_update === true) {
      ...
      return res.status(403).json({ status: "error", message: "User does not have update permission" });
    }
  } catch (err) {
    console.log(err);
    return res.status(500).json({ status: "error", message: "Failed to fetch permission" });
...
const checkcandelete = async (req, res, next) => {
  try {
    ...
    if (userPermission && userPermission.can_delete === true) {
      ...
      return res.status(403).json({ status: "error", message: "User does not have delete permission" });
    }
  } catch (err) {
    console.log(err);
    return res.status(500).json({ status: "error", message: "Failed to fetch permission" });
...
const isadmin = async (req, res, next) => {
  try {
    ...
    console.log(err);
    return res.status(500).json({ status: "error", message: "Failed to validate admin access" });
  }
};
